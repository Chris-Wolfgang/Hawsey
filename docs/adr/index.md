...
Once an ADR is **Accepted**, its content doesn't change. To reverse a decision,
write a *new* ADR. The only edit allowed on the old one is its **Status** line
(`Superseded by …`), and it stays in place.

| # | Title | Status |
|---|-------|--------|
| [0001](0001-record-architecture-decisions.md) | Record architecture decisions | Accepted |
| [0002](0002-ui-agnostic-engine-with-immutable-state.md) | A UI-agnostic engine that returns a new GameState per move | Accepted |
